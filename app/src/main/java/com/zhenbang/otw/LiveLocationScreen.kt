                                ...
                                items(otherUsers.toList(), key = { (userId, _) -> userId }) { (userId, otherUserLocation) ->
                                    ...
                                    UserListItem(
                                        ...
                                        onChatClick = {
                                            ...
                                            // TODO: Replace with your actual navigation route logic
                                            // navController.navigate("messaging_screen/$userId")
                                            // Assuming you have a Routes object or similar:
                                            // navController.navigate(Routes.messagingWithUser(userId))
                                            Toast.makeText(context, "Navigate to chat with $userName", Toast.LENGTH_SHORT).show() // Placeholder
                                        }
                                    )
...
@Composable
fun UserListItem( name: String, distanceText: String?, onClick: () -> Unit, onChatClick: () -> Unit ) { /* ... implementation unchanged ... */
    Row( modifier = Modifier.fillMaxWidth().padding(vertical = 8.dp, horizontal = 16.dp), verticalAlignment = Alignment.CenterVertically ) { Column( modifier = Modifier.weight(1f).clickable(onClick = onClick).padding(end = 8.dp) ) { Text(text = name, style = MaterialTheme.typography.bodyLarge); if (distanceText != null) { Text(text = "Distance: $distanceText", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant) } }; IconButton(onClick = onChatClick) { Icon( imageVector = Icons.Filled.MailOutline, contentDescription = "Chat with $name", tint = MaterialTheme.colorScheme.primary ) } }
}
...
private fun distanceBetween(p1: LatLng, p2: LatLng): Float { /* ... */ val results = FloatArray(1); try { android.location.Location.distanceBetween(p1.latitude, p1.longitude, p2.latitude, p2.longitude, results) } catch (e: Exception) { Log.e("DistanceCalc", "Error calculating distance", e); return Float.MAX_VALUE }; return results[0] }
